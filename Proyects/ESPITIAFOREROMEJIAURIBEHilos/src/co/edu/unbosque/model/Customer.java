...
public class Customer extends Thread {
	...
	public void run() {
		while (indx < Bank.getAtms().length) {
			System.out.println("Antes "+indx);
			System.out.println(indx +" "+Bank.getAtms()[indx].isOccupied());
			...
			if (Bank.getAtms()[indx].isOccupied() && !Bank.getAtms()[indx+1].isOccupied()) {
				indx++;
				System.out.println("Despues "+ indx);
;
			}
	...
	private String deposit() {
		b_account.setBalance(b_account.getBalance() + getT_value());
		return "+ Consignación por valor de $"+getT_value()+" completada exitosamente!!\nSiguiente.";
	}
